...
#'
#' @param model \code{lm}, \code{glm}, or \code{merMod} object.
#' @param glm.predict type of predictions to use when model is a \code{glm} object.
#' Can be "response" or "link", but only "response" is currently used for outputs.
#' @param glm.resid type of residuals to use when model is a \code{glm} or \code{glmerMod} object.
#' Can be "deviance", "pearson", "working", "response", or "partial".
#' @param smooth_method method of smoothing, which is passed to \code{\link[ggplot2]{geom_smooth}}. Default is "loess".
...
model_check_plots <- function(model,
  ...
  if("lm" %in% class(model)){
    ...
    aug <- broom::augment(model,
                          type.predict = glm.predict,
                          type.residuals = glm.resid) #glm. arguments ignored for lm objects
